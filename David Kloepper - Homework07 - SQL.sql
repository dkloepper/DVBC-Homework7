...
SELECT last_name, count(last_name) AS actor_count FROM actor GROUP BY last_name;

-- 4b. List last names of actors and the number of actors who have that last name, but only for names that are shared by at least two actors
SELECT last_name, count(last_name) AS actor_count FROM actor GROUP BY last_name HAVING count(last_name) > 1;
...
SELECT CONCAT(customer.first_name," ",customer.last_name) AS customer_name, sum(payment.amount) as total_payments 
FROM customer 
...
WHERE (film.title LIKE "K%" OR film.title LIKE "Q%") 
	AND film.language_id = (SELECT language.language_id FROM language WHERE language.name = "English");
...
SELECT CONCAT(customer.first_name," ",customer.last_name) as customer_name, customer.email as email_address 
FROM customer 
